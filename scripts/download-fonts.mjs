#!/usr/bin/env node
/**
 * Downloads Google Fonts as woff2 files to <app>/public/fonts/
 * and generates <app>/public/brand/fonts.css with @font-face declarations.
 *
 * Run from the consuming app's root (cwd): node brand-kit/scripts/download-fonts.mjs
 * Font list comes from --config <path> (default: ./brand.config.mjs relative to cwd).
...
const configPath = configFlagIndex !== -1
  ...
  : path.resolve(process.cwd(), 'brand.config.mjs')

const { default: config } = await import(pathToFileURL(configPath).href)

const FONTS_DIR = path.resolve(process.cwd(), 'public', 'fonts')
const CSS_OUT   = path.resolve(process.cwd(), 'public', 'brand', 'fonts.css')
const FONT_DEFS = config.fonts
...
function generateCSS(decls) {
  ...
  let css = `/* ================================================================
   ...
   Generated by brand-kit/scripts/download-fonts.mjs — do not edit by hand.
